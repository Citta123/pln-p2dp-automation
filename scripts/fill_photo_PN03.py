...
def fill_photo_PN03(data):
    base_dir = os.path.dirname(__file__)
    logger = setup_logger(os.path.join(base_dir, '..', 'log', 'project.log'))
    photo_path = os.path.join(base_dir, '..', 'photos', 'PN03', '3.jpg')

    if not os.path.exists(photo_path):
        logger.error(f"Foto PN03 tidak ditemukan di {photo_path}")
    ...
    try:
        ...
        for entry in data:
            ...
            namafoto_PN03 = generate_namafoto(blth, idpel, "PN03")
            entry['namafoto_PN03'] = namafoto_PN03
            entry['filefoto_PN03'] = encoded_string
            entry['transaksiby'] = entry.get('transaksiby', '52260.d08')  # Pastikan 'transaksiby' ada

        logger.info("Foto PN03 berhasil diisi ke data.")
    except Exception as e:
        logger.error(f"Error saat mengonversi foto PN03: {e}")
