...
def fill_photo_PN01(data):
    base_dir = os.path.dirname(__file__)
    logger = setup_logger(os.path.join(base_dir, '..', 'log', 'project.log'))
    photo_path = os.path.join(base_dir, '..', 'photos', 'PN01', '1.jpg')

    if not os.path.exists(photo_path):
        logger.error(f"Foto PN01 tidak ditemukan di {photo_path}")
    ...
    try:
        ...
        for entry in data:
            ...
            entry['namafoto_PN01'] = namafoto_PN01
            entry['filefoto_PN01'] = encoded_string
            entry['transaksiby'] = entry.get('transaksiby', '52260.d08')  # Pastikan 'transaksiby' ada

        logger.info("Foto PN01 berhasil diisi ke data.")
    except Exception as e:
        logger.error(f"Error saat mengonversi foto PN01: {e}")
